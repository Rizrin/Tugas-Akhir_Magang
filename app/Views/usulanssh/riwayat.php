...
<div style="display: flex; justify-content: space-between; align-items: flex-end;" class="page-title-box">
    <div>
        <h1 class="page-title">Riwayat Pengajuan SSH</h1>
        <p class="page-subtitle">Kelola dan pantau seluruh status usulan SSH yang telah diajukan.</p>
    </div>
    <div style="display: flex; gap: 10px;">
        <a href="<?= base_url('usulanssh/exportPdf') ?>" class="btn btn-outline" style="display:flex; align-items:center; gap:5px;"><i class='bx bx-download'></i> Unduh Laporan PDF</a>
        <a href="<?= base_url('usulanssh/exportExcel') ?>" class="btn btn-primary" style="display:flex; align-items:center; gap:5px;"><i class='bx bx-grid-alt'></i> Ekspor Data Excel</a>
    </div>
</div>
...
<div class="card">
    <form method="get" action="<?= base_url('usulanssh/riwayat') ?>" style="display: flex; gap: 15px; margin-bottom: 20px;">
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="8" style="text-align: center; color: var(--text-muted); padding: 30px;">Belum ada riwayat pengajuan SSH.</td>
                </tr>
                <?php else: ?>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; font-size: 0.85rem; color: var(--text-muted)">
        <div>Menampilkan <strong>1- <?= count($riwayat) ?></strong> dari <strong><?= count($riwayat) ?></strong> usulan SSH</div>
