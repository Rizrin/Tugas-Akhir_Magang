...
<div style="display: flex; justify-content: space-between; align-items: flex-end;" class="page-title-box">
    <div>
        <h1 class="page-title">Manajemen Draf</h1>
        <p class="page-subtitle">Kelola dan lanjutkan usulan SBU yang sedang dalam proses pengerjaan.</p>
    </div>
    <div style="display: flex; gap: 10px;">
        ...
        <a href="<?= base_url('usulan/input') ?>" class="btn btn-primary" style="display:flex; align-items:center; gap:5px;"><i class='bx bx-plus'></i> Usulan Baru</a>
    </div>
</div>

<div class="card">
    <form method="get" action="<?= base_url('usulan/draft') ?>" style="display: flex; gap: 15px; margin-bottom: 20px;">
    ...
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                <?php if (empty($drafts)): ?>
                <tr>
                    <td colspan="7" style="text-align: center; color: var(--text-muted); padding: 30px;">Tidak ada draft usulan yang ditemukan.</td>
                </tr>
                <?php else: ?>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; font-size: 0.85rem; color: var(--text-muted)">
        <div>Menampilkan <strong>1- <?= count($drafts) ?></strong> dari <strong><?= count($drafts) ?></strong> draf usulan</div>
